// content_type_value_test.mjs - oracle gate for the Lumen-native Content-Type header value
// extractor.
//
// The kernel (examples/http/content_type_value.lm) is pure Lumen protocol logic over a
// raw-memory byte buffer. This harness plays the role of the I/O seam that will later be a
// socket: it writes a fixture request's bytes into the compiled program's memory at REQ_BASE
// and the byte count at REQ_LEN_ADDR, runs the program, and asserts the bytes emitted at
// OUT_BASE (length read from OUT_LEN_ADDR) match the expected Content-Type value.
// Deterministic, offline.
//
// Because the kernel adds no compiler feature (only load8/store8/load32/store32 +
...
const SRC = fs.readFileSync(new URL('../examples/http/content_type_value.lm', import.meta.url), 'utf8');
const REQ_BASE = 600000;
const REQ_LEN_ADDR = 599996;
const OUT_BASE = 700000;
const OUT_LEN_ADDR = 699996;

// Each case: [raw request, expected Content-Type value string]
const CASES = [
...
let fail = 0;
console.log('== Lumen-native Content-Type header value extractor (oracle gate) ==');
for (const [raw, want] of CASES) {
  const got = await extract(raw);
  const ok = got === want;
...
console.log(fail === 0
  ? `\n${CASES.length}/${CASES.length} requests parsed correctly by the Lumen Content-Type extractor.`
