// http_keepalive_test.mjs - oracle gate for the Lumen-native HTTP/1.1 keep-alive decision.
//
// The kernel (examples/http/http_keepalive.lm) is pure Lumen protocol logic over a raw-memory
...
// count at REQ_LEN_ADDR, runs the program, and asserts the printed keep-alive decision (1 or 0)
// matches the expected value. Deterministic, offline.
//
...
const SRC = fs.readFileSync(new URL('../examples/http/http_keepalive.lm', import.meta.url), 'utf8');
const REQ_BASE = 600000;
const REQ_LEN_ADDR = 599996;

// Each case: [raw request, expected keep-alive int]
const CASES = [
  ['GET / HTTP/1.1\r\n\r\n', 1],
  ['GET / HTTP/1.1\r\nConnection: close\r\n\r\n', 0],
  ['GET / HTTP/1.0\r\n\r\n', 0],
...
async function keepalive(raw) {
  ...
  return Number(I.getOut().trim());
}

let fail = 0;
console.log('== Lumen-native HTTP/1.1 keep-alive decision (oracle gate) ==');
for (const [raw, want] of CASES) {
  const got = await keepalive(raw);
  const ok = got === want;
...
console.log(fail === 0
  ? `\n${CASES.length}/${CASES.length} requests decided correctly by the Lumen keep-alive kernel.`
